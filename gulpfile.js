...
, { babel: rollupBabel } = require('@rollup/plugin-babel')
, fs = require('fs')
, path = require('path')
;
 ...
 // For Prodaction
    ...
    const jsSourceMap = false; // main.js.map — ввімкни false щоб вимкнути
...
//JS
    gulp.task('js', function(){
        const outDir = 'build/js';
        if (!fs.existsSync(outDir)) fs.mkdirSync(outDir, { recursive: true });
        return rollup.rollup({
            input: 'front/js/main.js',
            plugins: [
                rollupBabel({
